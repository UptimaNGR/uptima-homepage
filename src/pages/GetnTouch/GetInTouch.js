      ...
      <Container>
        <div className="banner-content">
          ...
          <div className="banner-text">
            <h4>Talk to Us</h4>
            <p>Feel free to reach out to us, our team is here to answer your questions. Kindly fill out the form below and we’ll be in touch.</p>
          </div>
        </div>
              ...
              <form onSubmit={handleSubmit(onSubmit)}>
                {success && (
                  <div className="message-container">
                    <span className="success-message">Thank you for reaching out to us. We would get back to you shortly</span>
                ...
                <div className="form-control">
                  <div className="input-container">
                    <input type="text" placeholder="Enter Company Name" name="companyName" ref={register} />
                    {errors.companyName && <ErrorAnimation>{errors.companyName.message}</ErrorAnimation>}
                  </div>
                  <div className="input-container">
                    <input type="text" placeholder="Enter Phone Number" name="phoneNumber" ref={register} />
                    {errors.phoneNumber && <ErrorAnimation>{errors.phoneNumber.message}</ErrorAnimation>}
                  </div>
                </div>
                <div className="form-control">
                  <div className="input-container">
                    <textarea placeholder="Describe what you're building" name="message" ref={register}></textarea>
                    {errors.message && <ErrorAnimation>{errors.message.message}</ErrorAnimation>}
                  </div>
                </div>
                {posting ? <Spinner borderWidth="3px" size="30px" color="#050384" /> : <button>SEND</button>}
              </form>
            </div>
          </div>
        </div>
